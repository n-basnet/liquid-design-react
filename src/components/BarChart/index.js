...
export class BarChart extends PureComponent {
  ...
  setTooltipData = tooltipData => () => {
    ...
    this.setState({ tooltipData: currentTooltipId === tooltipData.id ? null : tooltipData })
  }
  setBarWidth = () => {
  ...
  render() {
    const { yTickFormatter, valueFormatter, topOffset, data: dataFromProps, ...props } = this.props
    const { data, labels, selectedLabelsIds, tooltipData, barWidth } = this.state
    return (
      <BarChartWrapper {...props}>
        <YTicks data={data} tickFormatter={yTickFormatter} topOffset={topOffset} />
        <BarGroupsWrapper
          data={data}
