...
import EnhancedHeadline, {
  Headline,
  HEADLINE_TYPES,
} from '../../src/elements/Headline'
...
const description = `
  ...
  The possible \`type\`s of headlines are: ${HEADLINE_TYPES.map(
    v => `\`${v}\``,
  ).join(', ')}, where \`B\` denotes a "branded" headline that uses the M Font.
`.trim()
